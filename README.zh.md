...
Kaji 也可以从悬浮窗启动本地 PetHatch runtime。它会依次查找
`KAJI_PETHATCH_ROOT`、`defaults write dev.kaji petHatchRoot /path/to/pethatch`
和 `~/workspace/pethatch` 等常见开发路径。Settings 窗口可以选择小柴或
Openclaw；正式 runtime discovery 后续也可以放在那里。通过 Kaji 启动时，
会把同一个 `pet-state.json` 传给 PetHatch，所以宠物能响应 quota 压力，而不只是独立计时器。
