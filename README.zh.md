...
- **设置窗口**：风格、已用/剩余、S/M 尺寸、中英文和 PetHatch 宠物选择等低频偏好；非商业宠物会标 `NC`。
...
Openclaw。PetHatch 更新后，可以在 Settings 里手动刷新本地 registry。它也会读取
每只宠物 `pet.json` 里的 license 信息，并把非商业资产标成 `NC`，避免选择时忽略授权边界。通过
