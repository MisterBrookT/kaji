...
// MARK: - ProviderLogo
//
// The center mark of each ring. These are the same vector shapes the design
// preview uses, so the native gauge stays faithful to the approved mock:
//   - radial burst
//   - knot mark
//   - four-point spark
...
struct ProviderLogo: View {
            ...
            case "ark-agent":
                ArkDiamond()
                    .stroke(color, style: StrokeStyle(lineWidth: size * (1.9 / 24),
                                                      lineJoin: .round))
            case "cursor":
                // Official brand mark (Simple Icons `cursor` / cursor.com hex wedge).
    ...
    /// Even-odd fill keeps the brand cutout; isolated so `Group` type-checks.
    @ViewBuilder
    private func cursorMark(color: Color) -> some View {
        VectorLogo(pathData: BrandPaths.cursor)
            .fill(color, style: FillStyle(eoFill: true))
    }
}

// MARK: - Faceted diamond
...
// MARK: - Radial burst
//
// 12 radial lines from a small inner radius outward, alternating length.
struct RadialBurst: Shape {
    func path(in rect: CGRect) -> Path {
        var p = Path()
        let s = min(rect.width, rect.height) / 24
        let cx = rect.midX, cy = rect.midY
...
// MARK: - Vector path data
//
// Verbatim from the design preview so the native gauge and the HTML mock render
// identical marks.
enum BrandPaths {
    // Knot mark.
    ...
    // Cursor brand mark (Simple Icons `cursor`, same hex-wedge as cursor.com/brand).
    ...
    // Geometric M monogram (sans-serif, V cut from top, flat bottom).
